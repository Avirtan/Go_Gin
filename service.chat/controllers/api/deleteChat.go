package ChatController

import (
	"avirtan/work_craft/config"
	"net/http"

	"github.com/gin-gonic/gin"
)

// Chat godoc
// @Summary 	Chat
// @Description delete chat by json
// @Tags 		Chat
// @Accept 		json
// @Produce 	json
// @Param 		chat body Chat true "chat"
// @Success		200 {string} string
// @Failure		400 {integer} http.StatusBadRequest
// @Security 	ApiKeyAuth
// @Router 		/delete [delete]
func DeleteChat(ctx *gin.Context) {
	var json Chat
	if err := ctx.ShouldBindJSON(&json); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	if !config.DB_Redis.SIsMember(ChatsKey, json.Name).Val() {
		ctx.JSON(http.StatusBadRequest, gin.H{"err:": "channels not exist"})
		return
	}
	if err := config.DB_Redis.SRem(ChatsKey, json.Name).Err(); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"err:": err})
		return
	}
	ctx.JSON(http.StatusOK, gin.H{"success": "chat delete"})
}
